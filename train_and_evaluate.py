...
class Trainer:
    ...
    def load_graph_information(self):
        ...
        missing_indices_dict = MissingDataMasking(self.feature_num, subgroups, self.config_idx).missing_indices_dict
    ...
    def train_validation_split(self, non_missing):
        if self.args.sampling_ratio == 1.0:
            ...
            sampler = NodeSampler(self.subgroups, self.config_idx, non_missing, self.args.sampling_ratio,
                                  Sampling.method)
            train_indices = sampler.selected_samples  # Todo: do i want to split this into train/val
    ...
    def execute(self):
        try:  # try loading pretrained models
            ...
            test_indices = {subgroup: self.missing_indices_dict[subgroup]['all'] for subgroup in self.subgroups}
            results_dict = {}
            for seed in range(1, self.seeds_num + 1):
                ...
                for subgroup in self.subgroups:
                    ...
                    model.to(device)
                    model.eval()
                    results_dict[seed][subgroup] = self.test(test_indices[subgroup], model, subgroup)
            ...
            for seed in range(1, self.seeds_num + 1):
                ...
                for subgroup in self.subgroups:
                    ...
                    test_indices = self.missing_indices_dict[subgroup]['all']
                    ...
                    results_dict[seed][subgroup] = self.test(test_indices, model, subgroup)
